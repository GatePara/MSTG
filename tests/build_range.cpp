...
{
    std::cout << "Executing build_range.cpp" << std::endl;
    try
    {
        ...
        bool range_filter = true;
         MultiSegmentTreeGraph::MSTGIndex<float> index(&storage, config.M, config.ef_construction, config.threads, range_filter);
